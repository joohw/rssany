// 语鲸 Open API 采集器：将语鲸推荐文章 / 今日文章转换为条目列表
//
// sourceId 格式：
...
function resolveCredentials(sourceId) {
  ...
  if (!appId || !appSecret) {
    throw new Error(
      "[LingowhaleCollector] 缺少认证信息：请在 sourceId 中提供 ?app_id=&app_secret= 参数，" +
...
async function fetchArticles(appId, appSecret, maxPages) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    if (!res.ok) throw new Error(`[LingowhaleCollector] HTTP ${res.status} 拉取文章列表失败`);
    const json = await res.json();
    if (json.code !== 0) throw new Error(`[LingowhaleCollector] API 错误：${json.message}`);
...
async function fetchTodayArticles(appId, appSecret) {
  ...
  for (let round = 0; round < 10; round++) {
    ...
    if (!res.ok) throw new Error(`[LingowhaleCollector] HTTP ${res.status} 拉取今日文章失败`);
    const json = await res.json();
    if (json.code !== 0) throw new Error(`[LingowhaleCollector] API 错误：${json.message}`);
    const pageItems = json.data.items ?? [];
    items.push(...pageItems.map(mapArticle));
