...
import * as rssCollector from "../app/collectors/builtin/rss.rssany.js";

describe("RSS collector arXiv fallback", () => {
  it("uses the official API when a category RSS feed is valid but empty", async () => {
    ...
    const items = await rssCollector.fetchItems("https://rss.arxiv.org/rss/cs.AI", ctx);
  ...
  it("does not fall back for a non-arXiv empty feed", async () => {
    ...
    const items = await rssCollector.fetchItems("https://example.com/feed.xml", ctx);
